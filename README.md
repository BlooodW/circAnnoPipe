...
This module generates prediction results from beRBP, downloads circRNA-RBP interaction data from ENCORI, and then merges the two results.

#### Output files

- **`results/RBP/beRBP.tsv`**
  Prediction result generated by beRBP in TSV format.
- **`results/RBP/ENCORI_hg38_RBPTarget_all_circRNA_HeLa.txt`**
  circRNA-RBP interaction data downloaded from the ENCORI database.
- **`results/RBP/merged_output.txt`**
  Final merged result integrating beRBP and ENCORI outputs.

### 3. Translation potential analysis

This module evaluates the translation potential of circRNAs using IRESfinder and ORFfinder.

#### Output files

- **`results/ORF/IRESfinder_mode_1.result`**
  IRESfinder output generated under mode 1.
- **`results/ORF/IRESfinder_mode_2.result`**
  IRESfinder output generated under mode 2.
- **`results/ORF/ORFfinder.fa`**
  ORFfinder output in FASTA format containing predicted ORF sequences.
